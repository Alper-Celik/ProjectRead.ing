...
public record UpdateWorkPayload(Queries.Work Work);
...
public record UpdateWorkInput : IBasicEntityMetadata
{
    ...
    {
        public UpdateWorkInputValidator(
            PGContext db,
        ...
        )
        {
            ...
            RuleFor(w => w.TagIds.Value!)
                .MustBeDistinct(nameof(TagIds))
                .WhenOptionalSet(w => w.TagIds);
            RuleFor(w => w.TagIds.Value!)
                .IdsMustExist(db.WorkTags, userId.Id)
                .WhenOptionalSet(w => w.TagIds);

            RuleFor(w => w.AuthorIds.Value!)
                .MustBeDistinct(nameof(AuthorIds))
                .WhenOptionalSet(w => w.AuthorIds);
            RuleFor(w => w.AuthorIds.Value!)
                .IdsMustExist(db.Authors, userId.Id)
                .WhenOptionalSet(w => w.AuthorIds);
        }
    }
    ...
    public static void ApplyToWork(Models.Work work, UpdateWorkInput input)
    {
        ...
        if (input.TagIds.HasValue)
            work.WorkTag_Works =
                input.TagIds.Value?.Select(t => new WorkTag_Work(work.Id, t)).ToList()
                ?? [];

        if (input.AuthorIds.HasValue)
            work.Work_Authors =
                input.AuthorIds.Value?.Select(t => new Work_Author(work.Id, t)).ToList()
                ?? [];
    }
}
